...
#    x_input = preprocess(x_input)
    ...
    x_batch.set_shape([a.batch_size,224,224,3])
...
with tf.name_scope('summary'):
    with tf.name_scope('Input_summaies'):
        tf.summary.image('x_batch', tf.image.convert_image_dtype(x_batch, dtype=tf.uint8, saturate=True))
        tf.summary.image('x_batch_r', tf.image.convert_image_dtype(x_batch_r, dtype=tf.uint8, saturate=True))
        tf.summary.image('x_batch_affine', tf.image.convert_image_dtype(x_batch_affine, dtype=tf.uint8, saturate=True))
...
fig = plt.figure()
y_mean = np.mean(clus_template, axis=0)
x_label = np.array([i for i in range(10)])
plt.bar(x_label, y_mean)
plt.title('Mean of y vector outputs')
plt.xlabel('N cluster')
plt.ylabel('mean')
plt.savefig(a.save_dir + '/' + 'mean_cluster.jpg')
plt.close(fig)
