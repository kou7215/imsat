...
parser.add_argument("--load_model_path", default='./logs', help="path for checkpoint")
...
with tf.Session(config=config) as sess:
    ...
    # test
        ...
        x_test_image = np.reshape(x_test, [-1,28,28])
        ...
        CLUSTER_SHOW_NUM = 16
        clustering_results_tmp = np.full((int(28*a.y_dim),int(28*CLUSTER_SHOW_NUM),3), 255, dtype=np.uint8)
        ...
        for i in range(len(x_test_image)):
            ...
            if cluster_count[clus] >= CLUSTER_SHOW_NUM:
                continue
            h, w = (int(28*clus), int(28*cluster_count[clus]))
            clustering_results_tmp[h:h+28, w:w+28,:] = img
            cluster_count[clus] += 1
        cv2.imwrite(a.save_dir + '/' + 'each_cluster.jpg', cv2.cvtColor(clustering_results_tmp,cv2.COLOR_RGB2BGR))
